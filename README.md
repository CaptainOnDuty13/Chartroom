...
- **Import KML** — drag and drop one or more `.kml` files. Each import produces one route per activity: a single-activity file becomes one track, and a bundled export (a Google Earth folder holding many activities) is split into one track per activity, each keeping its own name and original color. Raw per-sample GPS points and standalone markers — the "Track Points" folders and Start/End/Lap pins that Garmin exports include — are recognized and skipped.
...
- **Browse:** the sidebar groups tracks by trip or country (toggle at the top), sorted alphabetically throughout; each group starts collapsed — click to expand. The checkbox next to a group's name shows or hides every track in it at once.
...
- Altitude isn't captured from imported KML files (only latitude/longitude), so it won't appear in exports either, even if the original file had it. Coordinates are stored rounded to six decimal places (about 11 cm) — finer than GPS accuracy, but worth knowing if you need the raw values.
- Very large archives import fine, but a 30 MB+ file takes a little time to parse in the browser; the status message reports progress throughout.
- Every track's full geometry is loaded into the browser at startup — fine into the low thousands, but at roughly 13 kB per track this grows linearly (around 39 MB at 3,000 tracks). Well past 10,000 tracks, loading only the geometry for what's currently on screen would be the natural next step.
...
**1.3.1** — see the version comment at the top of `index.html`.

- 1.3.1 — **fixed a 1000-track ceiling on loading.** Supabase caps a single query at 1000 rows, so any archive larger than that was only partly loaded — the rest stayed in the database but never appeared in the app. Tracks are now fetched page by page with no practical limit, with a running count shown while loading.
- 1.3.0 — **large-archive imports now work.** Tracks are saved to the database in small batches instead of one request, so a multi-hundred-track file (previously silently rejected as a single oversized request) imports correctly. Added live import status — reading, parsing, archiving, and a running "saving N of M" count — plus a clear success or failure message at the end. If the original `.kml` can't be archived to storage, the tracks still import and the app says so rather than failing the whole import. Coordinates are stored rounded to ~11cm, well below GPS accuracy, which trims large files. Sidebar now sorts everything alphabetically (trips, countries, and tracks within each), using natural number ordering so "10" follows "9".
- 1.2.1 — a file that imports as a single route is now named after the file, rather than taking the generic placemark name (Garmin writes "Track") from inside it. Bundles still keep each activity's own name.
- 1.2.0 — **one route per import.** Standalone point markers (Garmin's Start / End / Lap pins, waypoints) are no longer imported as tracks; only actual paths are. A bundled export still yields one route per activity inside it. Also fixed batch group delete failing on large groups (the delete is now chunked, so a group of any size works).
- 1.1.3 — raw GPS sample points in a "Track Points" folder are no longer imported as hundreds of separate tracks; coordinates written with a space after the comma (`103.79, 1.28`) now parse correctly; sidebar collapse no longer blanks the map.
- 1.1.2 — first attempt at the sidebar-collapse map fix (superseded by the 1.1.3 CSS fix).
