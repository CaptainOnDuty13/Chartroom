...
- **Import KML** — drag and drop one or more `.kml` files. Bundled exports (a Google Earth "My Places" folder with many tracks in one file) are automatically split into individual tracks, each keeping its own name and original color.
...
**1.1.2** — see the version comment at the top of `index.html`.

- 1.1.2 — fixed the map going blank when the sidebar is collapsed/expanded on desktop (Leaflet wasn't being told its container had resized).
